...
Then apply in-flight migrations:
...
Verify schema: `select count(*) from information_schema.tables where table_schema='public';` — should be ~20 tables.
...
Also update PR #35 envs: `SPINE_SUPABASE_URL` + `SPINE_SUPABASE_SERVICE_ROLE_KEY` on the labs Coolify service (the dogfood card pulls from Spine's cloud).
